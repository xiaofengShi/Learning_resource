...
def main():
    ...
    rnn_param = {
        "rnn_input_size": rnn_input_size, "rnn_hidden_size": rnn_hidden_size,
        "rnn_layers": rnn_layers, "rnn_type": rnn_type, "bidirectional": bidirectional,
    ...
    params = {'num_epoches': num_epoches, 'end_adjust_acc': end_adjust_acc, 'seed': seed,
              'decay': decay, 'learning_rate': init_lr, 'weight_decay': weight_decay,
              'batch_size': batch_size, 'n_feats': train_dataset.n_feats}
    print(params)
    ...
    while not stop_train:
        ...
        loss = train(model, train_loader, loss_fn, optimizer,
                     logger, print_every=20, USE_CUDA=USE_CUDA)
    ...
    torch.save(
        CTC_Model.save_package(
            model, optimizer=optimizer, epoch=params, loss_results=loss_results,
