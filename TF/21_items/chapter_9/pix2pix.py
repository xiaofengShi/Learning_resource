...
parser.add_argument(
    "--display_freq", type=int, default=0,
...
parser.add_argument(
    "--lab_colorization", action="store_true",
...
parser.add_argument(
    "--scale_size", type=int, default=286,
...
def deconv(batch_input, out_channels):
    with tf.variable_scope("deconv"):
        ...
        filter = tf.get_variable(
            "filter", [4, 4, out_channels, in_channels],
            dtype=tf.float32, initializer=tf.random_normal_initializer(0, 0.02))
        # [batch, in_height, in_width, in_channels], [filter_width, filter_height, out_channels, in_channels]
        #     => [batch, out_height, out_width, out_channels]
        # 反卷积计算
        conv = tf.nn.conv2d_transpose(
            batch_input, filter, [batch, in_height * 2, in_width * 2, out_channels],
            [1, 2, 2, 1],
            padding="SAME")
        return conv
...
def rgb_to_lab(srgb):
    with tf.name_scope("rgb_to_lab"):
        ...
        with tf.name_scope("srgb_to_xyz"):
            ...
            rgb_pixels = (
                srgb_pixels / 12.92 * linear_mask) + (((srgb_pixels + 0.055) / 1.055) ** 2.4) * exponential_mask
        ...
        with tf.name_scope("xyz_to_cielab"):
            ...
            fxfyfz_pixels = (xyz_normalized_pixels / (3 * epsilon **
                                                      2) + 4/29) * linear_mask + (xyz_normalized_pixels ** (1/3)) * exponential_mask
...
def load_examples():
    ...
    with tf.name_scope("load_images"):
        ...
        assertion = tf.assert_equal(
            ...
            3, message="image does not have 3 channels")
        with tf.control_dependencies([assertion]):
    ...
    def transform(image):
        ...
        r = tf.image.resize_images(
            r, [a.scale_size, a.scale_size],
            method=tf.image.ResizeMethod.AREA)

        offset = tf.cast(tf.floor(
            tf.random_uniform(
                [2],
                0, a.scale_size - CROP_SIZE + 1, seed=seed)),
            dtype=tf.int32)
...
def create_model(inputs, targets):
    """ 判别器，原始图像进行卷积处理，将最后一层的sigmod结果作为概率 """
...
def main():
    ...
    with tf.name_scope("encode_images"):
        display_fetches = {"paths": examples.paths, "inputs": tf.map_fn(
            tf.image.encode_png, converted_inputs, dtype=tf.string,
            name="input_pngs"),
            "targets": tf.map_fn(
            tf.image.encode_png, converted_targets, dtype=tf.string,
            name="target_pngs"),
            "outputs": tf.map_fn(
            tf.image.encode_png, converted_outputs, dtype=tf.string,
            name="output_pngs"), }
        ...
        if a.mode == "test":
            ...
            for step in range(max_steps):
                ...
                if should(a.save_freq):
                    print("saving model")
                    saver.save(
                        sess, os.path.join(a.output_dir, "model"),
                        global_step=sv.global_step)
